...
@section('header_actions')
    <div class="header-actions-slot">
        <a href="{{ route('students.roster') }}" class="action-chip" title="Back to roster" aria-label="Back to roster">
...
@push('styles')
    <style>
        ...
        .admission-card { background:#fff; border:1px solid #d4ead4; border-radius:14px; overflow:hidden; }
        .admission-head { padding:14px 16px; border-bottom:1px solid #e7f3e7; display:flex; justify-content:space-between; gap:10px; flex-wrap:wrap; align-items:center; }
        .admission-head h3 { margin:0; font-size:24px; color:#1f3f24; font-weight:800; }
        .admission-body { padding:14px; }
        .error-box { margin-bottom:12px; border:1px solid #ffd6d6; background:#fff6f6; color:#8e2d2d; border-radius:10px; padding:10px 12px; font-size:13px; }
        ...
        .section-box { border:1px solid #e2efe2; border-radius:12px; padding:10px; background:#fcfffc; }
        .section-title { margin:0 0 8px; font-size:13px; font-weight:800; color:#1b5f2d; letter-spacing:.7px; text-transform:uppercase; padding-bottom:6px; border-bottom:2px solid #da1e28; }
        .field-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:9px; }
        .field { display:flex; flex-direction:column; gap:5px; }
        .field.full { grid-column:1 / -1; }
        .field label { font-size:11px; font-weight:700; color:#244f28; letter-spacing:.7px; text-transform:uppercase; }
        .field input, .field select, .field textarea { border:1px solid #dbe8fb; border-radius:8px; padding:9px 10px; font-size:14px; background:#fff; }
        .field textarea { min-height:74px; resize:vertical; }
        .field-hint { font-size:11px; color:#6f8570; margin-top:-1px; }
        .error { color:#bd3434; font-size:12px; font-weight:600; }
        .office-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:8px; }
        .check-item { display:flex; gap:8px; align-items:center; padding:8px; border:1px solid #e2efe2; border-radius:9px; background:#fff; font-size:12px; font-weight:700; color:#305433; }
        .check-item input { width:16px; height:16px; margin:0; }
        .submit { margin-top:12px; border:0; border-radius:10px; width:100%; padding:11px; background:linear-gradient(90deg,#0f7a35,#17a34a); color:#fff; font-weight:700; cursor:pointer; }
        @media (max-width:980px){ .section-grid{grid-template-columns:1fr;} .office-grid{grid-template-columns:1fr 1fr;} }
        @media (max-width:700px){ .field-grid,.office-grid{grid-template-columns:1fr;} }
    </style>
@endpush
...
@section('content')
    ...
    <section class="admission-card">
        <header class="admission-head">
            <h3>Edit Admission Form - {{ $student->student_code }}</h3>
            <strong>Form #: {{ old('form_number', $student->form_number ?: '-') }}</strong>
        </header>
        <div class="admission-body">
            <form method="POST" action="{{ route('students.update', $student) }}" enctype="multipart/form-data">
                ...
                <div class="section-grid">
                    ...
                    <section class="section-box">
                        <h4 class="section-title">Fee & Office Use</h4>
                        <div class="field-grid">
                        ...
                        <div class="office-grid" style="margin-top:10px;">
        ...
        (function () {
            ...
            if (emergencyInput) {
                emergencyInput.addEventListener('input', () => {
                    ...
                    const p1 = digits.slice(0, 4);
                    const p2 = digits.slice(4, 11);
                    emergencyInput.value = [p1, p2].filter(Boolean).join('-');
                });
            }
            ...
            document.querySelectorAll('.office-doc-input').forEach((fileInput) => {
                ...
                fileInput.addEventListener('change', () => {
                    ...
                    if (!f) {
                        if (fileInput.getAttribute('data-has-file') !== '1') {
                            cb.disabled = false;
                        }
                        return;
                    }
